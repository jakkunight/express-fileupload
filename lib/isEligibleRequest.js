...
const hasAcceptableContentType = (req) => {
  ...
  return ACCEPTABLE_CONTENT_TYPE.test(contType);
};
...
module.exports = (req) => {
  try {
    console.log(hasAcceptableMethod(req));
    console.log(hasAcceptableContentType(req));
    hasAcceptableMethod(req) && hasAcceptableContentType(req);
  } catch (e) {
    return false;
  }
};
